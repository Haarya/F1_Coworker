...
const RedBullCar = () => {
  ...
  useFrame((state) => {
    if (carRef.current) {
       ...
       carRef.current.position.y = -1 + Math.sin(state.clock.elapsedTime) * 0.05;
    }
  });
...
export const DriverStressMeter: React.FC<DriverStressMeterProps> = ({ stressScore }) => {
  ...
  return (
    <div className="relative w-full h-full min-h-[450px] bg-[#090909] rounded-2xl border border-white/5 flex flex-col items-center justify-center overflow-hidden shadow-[inset_0_0_40px_rgba(0,0,0,0.8)]">
    
      {/* Header */}
      <h2 className="absolute top-6 text-[10px] font-black uppercase tracking-[0.3em] text-white/30 z-20">Driver Stress Level</h2>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none mb-10">
        ...
        <div className="flex flex-col items-center justify-center relative z-20 bg-[#090909]/60 w-[120px] h-[120px] rounded-full backdrop-blur-sm border border-white/5">
          ...
          <span className="text-[9px] text-white/40 tracking-[0.2em] mt-1 font-bold">/ 100</span>
        </div>
      </div>
