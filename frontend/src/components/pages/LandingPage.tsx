...
export default function LandingPage() {
  ...
  // Navigate to /intro when the user attempts to scroll down while at the bottom of the page
  const [hasNavigated, setHasNavigated] = useState(false);
  useEffect(() => {
    let scrollAccumulator = 0;
    
    const handleWheel = (e: WheelEvent) => {
      if (mode === 'interactive' && !hasNavigated) {
        // Only trigger if at the very bottom and scrolling down
        if (window.innerHeight + window.scrollY >= document.documentElement.scrollHeight - 10 && e.deltaY > 0) {
          scrollAccumulator += e.deltaY;
          // Require a deliberate scroll amount (e.g. 150px) to prevent momentum/accidental redirects
          if (scrollAccumulator > 150) {
            setHasNavigated(true);
            navigate('/intro');
          }
        } else {
          // Reset accumulator if they scroll up or aren't at the bottom
          scrollAccumulator = 0;
        }
      }
    };
